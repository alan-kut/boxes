...
class Gravitrax(Boxes):
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--stand_units_number", action="store", type=int, default=4, help="Height of the stand in form of number of original stands")
        self.argparser.add_argument(
    ...
    def render(self):
        ...
        self.hexes()
        self.ctx.stroke()
        self.moveTo(1.5 *self.outer_l,0)
        self.stand()
        self.ctx.stroke()
        self.moveTo(self.stand_l +self.inner_snap_h + 1,0)
        self.stand()
        self.ctx.stroke()
        self.moveTo(self.stand_l +self.inner_snap_h + 1,0)
        self.stand()
    ...
    def stand(self):
        ...
        with self.saved_context():
            ...
            line.append(self.thickness) #Y-
            self.polyline(*line)
            text_x = -self.thickness - (self.stand_h - self.thickness * 3 - self.inner_snap_h)/2.0
            text_y = self.stand_l+self.inner_snap_h-self.stand_w + self.stand_w/2.0
            self.text(str(self.stand_units_number), color=Color.BLUE, fontsize=3, angle=90, x=text_x, y=text_y, align="center middle")
